...
class BassinConfig(Config):
    NAME = "bassin"
    IMAGES_PER_GPU = 4  # Adjust based on available GPU memory
    BATCH_SIZE=4
    NUM_CLASSES = 1 + 1  # Background + basin
    STEPS_PER_EPOCH = 142  # Assuming 143 images and batch size of 1
...
def main():
    ...
    if args.command == "train":
        model.load_weights(args.weights, by_name=True,exclude=["mrcnn_class_logits", "mrcnn_bbox_fc", "mrcnn_bbox", "mrcnn_mask"])
    else:
